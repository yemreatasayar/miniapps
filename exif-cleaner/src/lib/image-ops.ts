...
import { readMetadataSummary } from "./metadata";
import type { OutputFormat, SanitizableImage } from "./types";
...
function inferOutputFormat(file: File): OutputFormat {
  ...
  if (type.includes("png") || extension === "png") return "png";
  if (type.includes("webp") || extension === "webp") return "webp";
  return "jpg";
}
...
export async function loadSanitizableImage(file: File): Promise<SanitizableImage> {
  const [metadata, workingFile] = await Promise.all([readMetadataSummary(file), ensureBrowserReadableFile(file)]);
  ...
  try {
    ...
    return {
      ...
      fileSize: file.size,
      thumbnail,
      outputFormat: inferOutputFormat(file),
      metadata,
    };
  } finally {
    URL.revokeObjectURL(objectUrl);
...
export async function sanitizeImage(image: SanitizableImage): Promise<Blob> {
  ...
  try {
    ...
    return await canvasToBlob(canvas, getMimeType(image.outputFormat), image.outputFormat === "jpg" ? 0.92 : undefined);
  } finally {
    URL.revokeObjectURL(objectUrl);
  }
}

export function buildOutputFileName(image: SanitizableImage): string {
  const baseName = image.fileName.replace(/\.[^.]+$/, "");
  const extension = image.outputFormat === "jpg" ? "jpg" : image.outputFormat;
