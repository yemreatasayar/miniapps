...
- **Windows stem-helper installer'ı ZIP'ten ÇIKARTILARAK çalıştırılmalı.** `.cmd` kardeş `.ps1`'i `%~dp0` ile çağırır; ZIP içinden çalıştırılırsa `.ps1` temp'e çıkmaz → sessiz başarısızlık (eskiden yine "Installation completed" yazıyordu). `.cmd`'ye guard eklendi. Template değişikliği `windows-helpers.yml` ile yeni helper build'i gerektirir.
- **ffmpeg.wasm apps'i (video-to-audio, audio-editor) public'te COEP ile kırık — BEKLEMEDE.** `build-github-pages.mjs`'in SW'si COOP/COEP'i sadece navigation yanıtına ekliyor, ffmpeg worker'ına eklemiyor → `worker-BAOIWoxA.js` COEP ile bloklu, SharedArrayBuffer yok, `ffmpeg-core.worker.js` 404. Mac'te dev-server header'ları verdiği için çalışır; public site kırılgan. Kör düzeltme yok (çalışan Mac'i bozma riski); Windows'ta iteratif test gerekir. Detay: `notes/ui-standardization-and-windows-test.md`.
